...
type IconType = (Frontned | Backend|Database|Styling|State|Cloud|CiCd)[];
    ...
    <>
      {icon?.length > 0 &&
        icon.map((icon) => (
          ...
          <div className={cn("min-w-[4rem] min-h-[4rem] max-w-[4rem] max-h-[4rem] rounded-lg shadow-custom-border-3 flex justify-center items-center object-cover"
          )}>
            <Image
          ...
          <div className="text-[0.785rem] mt-2 text-[rgba(0,0,0,0.8)]">{icon}</div>
          </div>
        ))}
    </>
  );
